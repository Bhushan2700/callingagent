    ...
    <div style={{ padding: '2rem', maxWidth: 1000, margin: '0 auto', width: '100%' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          {[7, 30, 90].map(d => (
            <button key={d} onClick={() => setDays(d)} style={{ padding: '0.4rem 0.9rem', borderRadius: 8, border: '1px solid var(--glass-border)', cursor: 'pointer', fontSize: '0.8rem', fontWeight: 600, background: days === d ? 'var(--brand-gradient)' : 'transparent', color: days === d ? '#fff' : '#57A3AF' }}>{d}d</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
        <div style={{ background: 'var(--glass)', backdropFilter: 'blur(20px)', border: '1px solid var(--glass-border)', borderRadius: 16, padding: '1.25rem 1.5rem' }}>
          <h3 style={{ fontSize: '1rem', fontWeight: 700, marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Brain size={15} color="#41808B" /> Resolution breakdown</h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
              {Object.entries(data.resolution_breakdown).map(([k, v]) => (
                <div key={k} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: '0.82rem' }}>
                  <span style={{ width: 8, height: 8, borderRadius: 4, background: RESOLUTION_COLORS[k] || '#41808B', flexShrink: 0 }} />
                  <span style={{ textTransform: 'capitalize', color: 'var(--text-secondary)', minWidth: 150 }}>{resolveLabel(k, RESOLUTION_LABELS)}</span>
                  <div style={{ flex: 1, height: 8, borderRadius: 4, background: 'rgba(255,255,255,0.65)', overflow: 'hidden' }}>
                    <div style={{ width: `${(v / data.total_calls) * 100}%`, height: '100%', background: RESOLUTION_COLORS[k] || '#41808B', borderRadius: 4 }} />
                  </div>
                  <span style={{ color: '#41808B', fontWeight: 600, minWidth: 24, textAlign: 'right' }}>{v}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{ background: 'var(--glass)', backdropFilter: 'blur(20px)', border: '1px solid var(--glass-border)', borderRadius: 16, padding: '1.25rem 1.5rem' }}>
          <h3 style={{ fontSize: '1rem', fontWeight: 700, marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Wrench size={15} color="#7FB800" /> Tool usage</h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
              {data.tool_usage.map(t => (
                <div key={t.tool} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: '0.82rem' }}>
                  ...
                  <div style={{ flex: 1, height: 8, borderRadius: 4, background: 'rgba(255,255,255,0.65)', overflow: 'hidden' }}>
                    <div style={{ width: `${(t.count / maxTool) * 100}%`, height: '100%', background: 'linear-gradient(90deg, #57A3AF, #7FB800)', borderRadius: 4 }} />
                  </div>
                  <span style={{ color: '#41808B', fontWeight: 600 }}>{t.count}</span>
                </div>
              ))}
            </div>
          )}
      ...
      <div style={{ background: 'var(--glass)', backdropFilter: 'blur(20px)', border: '1px solid var(--glass-border)', borderRadius: 16, padding: '1.25rem 1.5rem' }}>
        <h3 style={{ fontSize: '1rem', fontWeight: 700, marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Activity size={15} color="#7FB800" /> Answer confidence over time</h3>
