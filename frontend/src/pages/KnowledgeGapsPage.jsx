...
const STATUS_META = {
  new: { color: '#fbbf24', label: 'New' },
  reviewing: { color: '#93c5fd', label: 'Reviewing' },
  resolved: { color: '#6ee7b7', label: 'Resolved' },
            ...
            return (
              <div key={g.id} style={{ background: 'var(--glass)', backdropFilter: 'blur(20px)', border: '1px solid var(--glass-border)', borderRadius: 14, padding: '1rem 1.25rem', display: 'flex', alignItems: 'center', gap: '0.9rem' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: '0.9rem', color: '#e2e8f0', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{g.question}</div>
                ...
                {g.status !== 'ignored' && g.status !== 'resolved' && (
                  <button onClick={() => changeStatus(g.id, 'resolved')} style={{ ...actionStyle, color: '#6ee7b7', borderColor: 'rgba(16,185,129,0.3)' }}>Resolve</button>
                )}
                {g.status !== 'ignored' && (
                  <button onClick={() => changeStatus(g.id, 'ignored')} style={{ ...actionStyle, color: '#94a3b8' }}>Ignore</button>
                )}
              </div>
            );
          })}
...
const actionStyle = { padding: '0.35rem 0.75rem', borderRadius: 8, border: '1px solid rgba(147,197,253,0.3)', background: 'transparent', color: '#93c5fd', fontSize: '0.75rem', cursor: 'pointer', flexShrink: 0 };
