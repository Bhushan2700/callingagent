...
function CallRow({ conv, onOpen }) {
  ...
  const color = RESOLUTION_COLORS[conv.resolution_status] || '#64748b';
  return (
    <button
      onClick={() => onOpen(conv)}
      style={{
    ...
    >
      ...
      <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: '#e2e8f0', fontWeight: 600 }}>
        {caller || 'Recent call'}
      </span>
  ...
  return (
    <div style={{ background: 'var(--glass)', backdropFilter: 'blur(20px)', border: '1px solid var(--glass-border)', borderRadius: 16, overflow: 'hidden' }}>
      <div style={{ padding: '1.1rem 1.25rem 0.75rem', borderBottom: '1px solid var(--glass-border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        {viewAllTo && <Link to={viewAllTo} style={{ fontSize: '0.75rem', color: '#93c5fd', textDecoration: 'none' }}>View all</Link>}
      </div>
...
export default function Dashboard() {
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: 1200, margin: '0 auto', width: '100%' }}>
      ...
      {/* Insight line */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '0.85rem 1.1rem', borderRadius: 12, background: 'rgba(20,184,166,0.07)', border: '1px solid rgba(20,184,166,0.2)', marginBottom: '1.5rem', fontSize: '0.9rem', color: 'var(--text-secondary)' }}>
        <CheckCircle2 size={16} color="#5eead4" style={{ flexShrink: 0 }} />
        <span>{insight}</span>
        {!insightDone && (
          <Link to="/voice" style={{ marginLeft: 'auto', color: '#5eead4', textDecoration: 'none', fontWeight: 600, fontSize: '0.82rem', flexShrink: 0 }}>
            Test voice agent →
          </Link>
        )}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(420px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
        <ChartCard icon={<Phone size={14} />} color="#5eead4" title="Calls Over Time">
          {daily.length === 0 ? (
        ...
        <ChartCard icon={<MessageSquare size={14} />} color="#93c5fd" title="What Customers Ask">
          {intentData.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={intentData} layout="vertical" margin={{ left: 0, right: 20 }}>
                ...
                <YAxis type="category" dataKey="name" tick={{ fontSize: 11, fill: '#cbd5e1' }} axisLine={false} tickLine={false} width={110} />
                <Tooltip contentStyle={chartTooltipStyle()} formatter={(v) => [v, 'Conversations']} />
                <Bar dataKey="count" fill="#60a5fa" radius={[0, 6, 6, 0]} maxBarSize={18} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </ChartCard>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
        <ListCard icon={<Users size={15} />} color="#93c5fd" title="Recent Calls & Conversations" viewAllTo="/calls" empty="No calls yet">
          {recent.map(c => <CallRow key={c.id} conv={c} onOpen={openConversation} />)}
        </ListCard>

        <ListCard icon={<Calendar size={15} />} color="#fbbf24" title="Upcoming Appointments" viewAllTo="/appointments" empty="No upcoming appointments">
          {upcoming.map((a, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: '0.82rem', padding: '0.55rem 0.7rem', color: 'var(--text-secondary)' }}>
              <span style={{ width: 8, height: 8, borderRadius: 4, background: '#fbbf24', flexShrink: 0 }} />
              <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                <span style={{ color: '#e2e8f0', fontWeight: 600 }}>{a.customer_name || a.customer_email || 'Guest'}</span>
              ...
              {a.source === 'ai' && <span style={{ fontSize: '0.62rem', fontWeight: 700, color: '#5eead4', flexShrink: 0 }}>AI</span>}
            </div>
          ))}
        </ListCard>
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', marginBottom: '2rem' }}>
        ...
        <Link to="/voice" style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '0.55rem 1rem', borderRadius: 10, border: '1px solid rgba(94,234,212,0.4)', background: 'rgba(20,184,166,0.1)', color: '#5eead4', fontSize: '0.85rem', fontWeight: 600, textDecoration: 'none' }}>
